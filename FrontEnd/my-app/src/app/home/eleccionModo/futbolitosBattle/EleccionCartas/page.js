...
export default function futbolitosBattle() {
    ...
    const [contador, setContador] = useState(0);  // Cambié 'contador' a estado con useState
    ...
    useEffect(() => {
        ...
        socket.on("Jugadores Listos", (data) => {
            if (data.UserId !== userID) {
                setContador((prevContador) => prevContador + 1);  // Uso de setContador para actualizar el estado
                setMensajeEstado("El otro... esta LISTO");
            }
        });
    }, [socket, isConnected]);
    ...
    useEffect(() => {
        if (contador === 2) {  // Verificación de si el contador llegó a 2
            console.log(equipo);
            toggleMode();
            setBloqueado(true);
        }
    }, [contador]);  // Dependemos de 'contador' para que se ejecute este efecto cuando cambie
